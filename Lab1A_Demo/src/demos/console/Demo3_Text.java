...
// http://www.java2s.com/Tutorials/Java/Java_Regular_Expression/0070__Java_Regex_Groups.htm    
    /**
     * ******************************************************
     * Toliau ekrane pateikiama eilutė, kur iki simbolių <> Regex Patternas o
     * toliau seka pagal šį šabloną apdorojamas tekstas po pirmo bandymo
     * pakeiskite šabloną kad būtų atpažintas minusas sekančių keitimų metu
     * atpažinkite: a - žodžius su lotyniškomis raidėmis b - žodžius su
     * lietuviškomis raidėmis c - informaciją tarp skliaustų ()
    **************************************************
     */
    void regexExamples() {
        if (ta1.getText().isEmpty()) // jei tuščia - pateikiame demo variantą
    ...
    void regexExampleMinus() {
        if (ta1.getText().isEmpty()) // jei tuščia - pateikiame demo variantą
        {
            ta1.appendText("[-]+<>MINUSsdfAS----MI65NUS----" + nL);
        }
        String[] sa = readLastLine().split("<>");      // padaliname į dvi dalis
        ta2.appendText(String.join(" : ", sa) + nL); // pakartojame dešinėje
        Pattern pat = Pattern.compile(sa[0]);      // pirma dalis - tai Pattern
        ...
        if (ta1.getText().isEmpty()) // jei tuščia - pateikiame demo variantą
        {
            ta1.appendText("\\p{IsLatin}*\\p{IsLatin}<>LOTYNI123-ŠKI *RAŠ2M3EN1YS" + nL);
        }
        String[] sa = readLastLine().split("<>");      // padaliname į dvi dalis
        ta2.appendText(String.join(" : ", sa) + nL); // pakartojame dešinėje
        Pattern pat = Pattern.compile(sa[0]);      // pirma dalis - tai Pattern
    ...
    void regexExampleLithuanianLetters() {
        if (ta1.getText().isEmpty()) // jei tuščia - pateikiame demo variantą
        {
            ta1.appendText("\\p{IsLatin}*[\u0080-\u9fff]+\\p{IsLatin}*"
                    + "<>LIETUVIŠKI 123123 RAŠMENYS 0- ŠĄČĘąčęėįęėš<>įtįę" + nL);
        }
        String[] sa = readLastLine().split("<>");      // padaliname į dvi dalis
        ta2.appendText(String.join(" : ", sa) + nL); // pakartojame dešinėje
        Pattern pat = Pattern.compile(sa[0]);      // pirma dalis - tai Pattern
